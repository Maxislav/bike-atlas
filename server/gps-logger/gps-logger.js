...
class Logger {
    ...
    emitUnlockUser(emitUnlockUser, device) {
        ...
        this.util.getOwnerDevice(device.device_key)
            ...
            if (areas) {
                const isInPrivate = distance_1.distance.isInPrivate(areas, device);
                if (!isInPrivate) {
    ...
    parseGprmc(gprmc, id) {
        ...
        if (id === '222222222222') {
            date = new Date(year, Number('' + dateStamp[2] + dateStamp[3]) - 4, //month
            ...
            date = new Date(year, parseFloat('' + dateStamp[2] + dateStamp[3]) - 1, Number('' + dateStamp[0] + dateStamp[1]), Number('' + timeStamp[0] + timeStamp[1]), Number('' + timeStamp[2] + timeStamp[3]), Number('' + timeStamp[4] + timeStamp[5]));
            lat = arrData[4] === 'N' ? minToDec(arrData[3]) : '-' + minToDec(arrData[3]);
            lng = arrData[6] === 'E' ? minToDec(arrData[5]) : '-' + minToDec(arrData[5]);
            azimuth = parseFloat(Number(arrData[8]).toFixed(2));
            speed = parseFloat(arrData[7]) * 1.852;
...
//# sourceMappingURL=gps-logger.js.map