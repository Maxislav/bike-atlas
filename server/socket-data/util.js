...
class Util {
    ...
    addDeviceBySocketId(socket_id, device) {
        return this.getUserIdBySocketId(socket_id)
            .then(user_id => {
            return new Promise((resolve, reject) => {
                this.connection.query('INSERT INTO `device` (`id`, `user_id`, `device_key`, `name` ,`phone`) VALUES (NULL, ?, ?, ?, ?)', [user_id, device.device_key, device.name, device.phone], (err, results) => {
                    if (err) {
                        reject(err);
                    }
                    resolve(true);
            ...
            this.connection.query('INSERT INTO `strava` ' +
                ...
                if (err) {
                    if (err.code == 'ER_DUP_ENTRY') {
                        this.connection.query('UPDATE `strava` SET  `strava_client_id`=?, `atlas_token`=?, `strava_client_secret`=?, `date`=? WHERE user_id=?', [stravaClientId, atlasToken, stravaClientSecret, new Date(), userId], (err, result) => {
                            if (err) {
                                reject(err);
                                return;
                            }
        ...
        return this.checkExistPoint(deviceKey, point.lng, point.date)
            ...
            return new Promise((resolve, reject) => {
                this.connection.query('INSERT INTO `logger` ' +
                    '(`id`, `device_key`, `lng`, `lat`, `speed`, `date`) VALUES (NULL, ?, ?, ?, ?, ?)', [deviceKey, point.lng, point.lat, point.speed, new Date(point.date)], (err, results) => {
                    if (err) {
                        reject(err);
                        return;
                    }
        ...
        return new Promise((resolve, reject) => {
            ...
            if (id) {
                ...
                this.connection.query('INSERT INTO `marker` ' +
                    '(`id`, `user_id`, `lng`, `lat`, `title`, `date`) VALUES (NULL, ?, ?, ?, ?, ?);', [userId, lng, lat, title, new Date()], (err, results) => {
                    if (err) {
                        reject(err);
                        return;
                    }
